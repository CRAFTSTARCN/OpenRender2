...
[[Pass]] 
    ...
    [[GBufferFragment]]
        ...
        vec3 normal_map = texture(NormalMap, IN.uv).rgb;

        data.normal = ApplyGLNormal(normal_map, normalize(IN.world_normal), normalize(IN.world_tangent));
        data.subsurface = texture(Subsurface, IN.uv).r;
        data.emissive = Emissive.rgb;
        data.tangent = normalize(IN.world_tangent);
        data.anisotropy = Anisotropy;
        data.metallic = Metallic;
        data.specular = Specular;
        data.roughness = texture(Roughness, IN.uv).r;
